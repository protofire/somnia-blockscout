defmodule Explorer.Chain.Cache.Helper do
  ...
  @doc """
    ...
    Modification for Somnia:
    Previous query was providing a very off number when querying Somnia database due to the
    high transactions throughput. We updated to the new query bellow since it is a more
    accurate number, but still an approximation.
  ...
  def estimated_count_from(table_name, options \\ []) do
    %Postgrex.Result{rows: [[count]]} =
      Chain.select_repo(options).query!("SELECT n_live_tup FROM pg_stat_all_tables WHERE relname = '#{table_name}';")
