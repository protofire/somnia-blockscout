defmodule Explorer.Chain.Import.Runner.Blocks do
  ...
  defp lose_consensus(
    ...
    # On high-TPS chains (e.g. Somnia) the operations below join the massive transactions
    # table and are extremely expensive. When no blocks actually lost consensus (the normal
    # case — no chain reorg) all four operations would produce 0 rows, so we skip them
    # entirely. The FOR NO KEY UPDATE lock on blocks was already acquired above, preserving
    # the sharelock ordering invariant.
